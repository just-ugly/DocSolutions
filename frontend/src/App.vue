<template>
  <div id="app" class="basic_style" :class="theme">
    <div class="side_bar" :style="side_bar()">
      <div v-if="more_setting" class="more_setting_header">更多设置</div>
      <!-- 侧边栏 -->
      <div class="side_container">
        <Transition name="fade" mode="out-in">
          <div v-if="!more_setting" class="fade_content">
            <!-- 顶部 UI -->
            <div class="side_top">
              <!-- 用户头像 -->
              <div class="user_avatar">
                <img v-if="avatar_url" :src="avatar_url" alt="用户头像">
                <div v-else class="no_avatar"></div>
              </div>
              <!-- 用户名 -->
        ...
        <Transition name="fade" mode="out-in" @after-enter="onFadeInComplete">
          <div v-if="more_setting" class="fade_content">
            ...
            <div class="more_setting_title">面向受众</div>
            <select class="setting_select" v-model="facing">
              <option value="不指定">不指定</option>
              <option value="技术人员">技术人员</option>
              <option value="学生">学生</option>
              <option value="客户">客户</option>
              <option value="观众">观众</option>
              <option value="平时聊天对象">平时聊天对象</option>
            </select>
    ...
    <!-- 主界面 -->
    <main class="main_interface">
      <!-- 下半部分 -->
      <div class="bottom_interface">
        <div class="ask_interface">
          <!-- 输入框 -->
          <textarea class="main_question" v-model="question" @input="autoResize"/>
          <div class="button_area_under_question">
      ...
      <!-- 上半部分 -->
      <div class="chat_area" :style="chat_area()">
        <!-- 欢迎界面 -->
        <div v-if="welcome" class="welcome_sentence">
          您有什么想探讨的？
        </div>
        <!-- 聊天记录 -->
...
export default {
  ...
  methods: {
    ...
    side_bar() {
      return {
        width: this.more_setting ? '350px' : '250px',
        padding: this.more_setting ? '20px' : '30px 12px 12px 12px',
        background: 'var(--side_bg)',
        borderRight: '1px solid var(--side_border)',
        display: 'flex',
    ...
    onFadeInComplete() {
      this.$nextTick(() => {
        // 获取当前显示的面板内的所有 textarea
        const container = document.querySelector('.fade_content');
        if (!container) return;

        const textareas = container.querySelectorAll('.setting_textarea');
    ...
    async uploadAndAppend(file) {
      ...
      const index = this.docFiles.length; // 获取索引
      ...
      // 处理错误
      if (!res.ok) {
        console.error("发送请求失败, 文件名:", file.name, "，用户:", this.username)
        const text = await res.text();
        console.error("错误信息:", text);
        this.docFiles[index].uploading = false;
        this.docFiles[index].error = text;
        return null; // 不抛出错误
      }
      // 解析响应
      ...
      // 更新状态
      this.docFiles[index].upload_file_id = upload_file_id;
      this.docFiles[index].uploading = false;
      // 返回给 Promise 队列
    ...
    async send(create_docx = false, self_table = false) {
      ...
      console.log("已上传payload, conversation_id:" + payload.conversation_id);
      ...
      while (true) {
        ...
        // 解码流式数据并拼接,若为__RESULT__开头则为json
        const chunk = decoder.decode(value, {stream: true});
        if (!chunk.startsWith("__RESULT__")) {
          botContent += chunk;
        } else {
          let result = chunk.replace("__RESULT__", "")
          let result_json = JSON.parse(result);
          this.conversation_id = result_json.conversation_id
        }
    ...
    async copyText(text) {
      ...
      try {
        ...
        // 可以在这里添加提示，比如 console.log 或一个简单的 toast
        console.log('复制成功');
      } catch (err) {
...
.side_bar {
  ...
  padding: 20px;
  background: var(--side_bg);
  border-right: 1px solid var(--side_border);
  overflow: hidden;
...
.drop_zone {
  ...
  background: none;
  border: 2px solid var(--setting_textarea_border);
  border-radius: 8px;
  cursor: pointer;
  transition: all 0.1s ease;
}
