  ...
  methods: {
      ...
      try {
        ...
        if (res.body && res.body.getReader) {
          ...
          if (json.error) {
            ...
            // persist conversation id from non-streaming responses as well
            if (this.finalResult && this.finalResult.conversation_id) {
              this.currentConversationId = this.finalResult.conversation_id;
            }
    ...
    watch: {
      ...
      currentConversationId(newCid) {
        try {
          // Only persist non-empty, valid ids. Avoid overwriting a good stored id with null/empty on reload.
          if (newCid && newCid !== 'null' && newCid !== 'undefined' && String(newCid).trim() !== '') {
